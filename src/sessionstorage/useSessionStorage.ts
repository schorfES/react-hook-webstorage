import { WebStorageErrorEvent } from '../events/WebStorageErrorEvent';
import type { StorageProvider, UseWebStorageHookOptions, UseWebStorageHookResult } from '../useWebStorage';
...
const SessionStorageProvider: StorageProvider = {
  ...
  get storage() {
    try {
      ...
      //
      // Dispatch error event once this happens:
      const event = new WebStorageErrorEvent(this.name, error);
...
export function useSessionStorage<T = unknown>(
  ...
  return useWebStorage<T>(SessionStorageProvider, key, options);
}
