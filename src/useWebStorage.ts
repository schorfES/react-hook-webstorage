...
export type CustomEventDetail<T> = { value: StorageValue<T> };
...
function isCustomEvent<T>(event: Event): event is CustomEvent<CustomEventDetail<T>> {
  return event instanceof CustomEvent && event.detail?.value !== undefined;
}
 ...
 */
export function useWebStorage<T = any>(
  ...
  const setItem = useCallback((value: StorageValue<T>): void => {
    const { name } = provider;
    const init: CustomEventInit<CustomEventDetail<T>> = { detail: { value } };
    const event = new CustomEvent(`storage:${name}:update:${key}`, init);
  ...
  const removeItem = useCallback((): void => {
    const { name } = provider;
    const init: CustomEventInit<CustomEventDetail<T>> = { detail: { value: null } };
    const event = new CustomEvent(`storage:${name}:update:${key}`, init);
    window.dispatchEvent(event);
  ...
  useEffect((): () => void => {
    const { name } = provider;
    window.addEventListener(`storage:${name}:update:${key}`, onUpdateValue);
    window.addEventListener('storage', onUpdateStorage);

    return (): void => {
      window.removeEventListener(`storage:${name}:update:${key}`, onUpdateValue);
