  ...
  describe('client side', () => {
    ...
    it('should handle updates across other useLocalStorage hooks', () => {
      ...
      act(() => {
        ...
        const event = new CustomEvent('storage:local-storage:update:foo', init);
      ...
      window.addEventListener('storage:local-storage:update:foo', onUpdateFoo);
      window.addEventListener('storage:local-storage:update:bar', onUpdateBar);
      ...
      expect(onUpdateFoo).toHaveBeenCalledWith(
        new CustomEvent('storage:local-storage:update:foo', {
          detail: {
            value: 'notify me!',
          },
        }),
    ...
    it('should handle invalid updates attempts across other useLocalStorage hooks', () => {
      ...
      act(() => {
        // Wrong event type:
        const event = new MouseEvent('storage:local-storage:update:foo');
        window.dispatchEvent(event);
      });
      ...
      act(() => {
        ...
        const event = new CustomEvent('storage:local-storage:update:foo');
        window.dispatchEvent(event);
      });
